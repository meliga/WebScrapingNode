...
function cartola(error, response, body) {
    ...
    var hora      = $(tabela).find('div.full-game-time').eq(0).text();
    var mandante  = $(tabela).find('div.game-team-1').eq(0).text();
    var visitante = $(tabela).find('div.game-team-2').eq(0).text();
    var placar    = $(tabela).find('div.game-score').eq(0).text().trim().split("\n");

    var gols_mandante  = placar[0].trim();
    var gols_visitante = placar[2].trim();

    var local = $(tabela).find('div.full-game-location span').eq(0).text().trim().split("\n");
